...
\begin{abstract}
We introduce \textit{deep-field \textsc{metacalibration}}, a new technique that reduces
the pixel noise in \mcal estimators of weak lensing shear signals by taking advantage of
a smaller, deeper imaging survey for calibration. Standard \mcal estimators of weak
lensing shear signals apply small artificial shears to the observed images of galaxies
in order to estimate the response of the object's shape to shear. This response is
combined with an uncalibrated shape measurement to produce properly calibrated shear
measurements. Unfortunately, current standard \mcal algorithms for weak lensing shear
measurement double the pixel noise variance in the images due to a numerical correction
needed during the artificial shearing process. Our new deep-field \mcal technique
leverages a separate, deeper imaging survey to avoid this increase in the pixel noise. We
provide algorithms to apply this technique to surveys, describe how to generalize it to
shear estimators that rely explicitly on object detection (e.g., \mdet), and
demonstrate that weak lensing shear measurement with deep-field \mcal is unbiased up to
second-order shear effects. We further show that for a typical lensing source,
...
shape  measurement errors. The overall increase in the precision of weak lensing
measurements for the Rubin LSST will depend on the somewhat unknown details of the LSST
Deep Drilling Field observations in terms of area and depth, the relative point-spread
function properties of the DDF and main LSST surveys, and the relative contribution of
pixel noise versus intrinsic shape noise to the total shape noise in the survey. We
estimate conservatively at least a $\approx15\%$ overall increase in the precision of
weak lensing analyses due to the increased source density and reduced pixel noise
contributions to the overall shape noise. Finally, we show that the technique is robust
to sample variance in the Vera C. Rubin Observatory Legacy Survey of Space and Time
(LSST) Deep Drilling Fields (DDFs) due to their large area, with the equivalent
calibration error being $\lesssim0.1\%$. The deep-field \mcal technique provides higher
signal-to-noise weak lensing measurements while still meeting the stringent systematic
error requirements of future surveys.
\end{abstract}
...
typical signal-to-noise of $\approx19$ for the wide-field images. That's right.} The deep-field images
...
error on the mean shear relative to standard \mcal. This estimate roughly matches the
results of \citet{SheldonMcal2017} who found that the increased pixel noise in standard
\mcal resulted in an overall decrease of $\approx20$ in the precision of the weak
lensing survey. Our estimate is a rough lower bound on the increase in precision due to
deep-field \mcal. In more realistic scenarios, we may be able to use more of the DDF
...
\begin{itemize}
  ...
  \item When applied to the Rubin LSST, we expect an at least $\approx15\%$ increase in
    ...
    to the shape noise and increased effective source density.

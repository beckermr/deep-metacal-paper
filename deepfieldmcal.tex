...
this work. Second, in order to artificially shear an image, one must first deconvolve
...
are set to have 10$\times$ less variance than the wide-field images. 
\jz{discussion on image dim:} Both the wide and 
deep field images are generated on a $53\times53$ images with pixel size set to 0.2". 
This particular choice of pixel size is expected to reflect that of LSST surveys.  
\jz{check the unit for pixel size and reference?}. We find that our
...
\begin{table*}
  ...
  \begin{tabular}{cccccc}
    ...
    2 & variable & fixed    & exponential & $0.36\pm0.23$   & $-0.63\pm1.77$ \\
...
uniformly by a factor between 0.9 and 1.1. \jz{discussion on image dim:} Unlike the previous cases, we simulate the deep
and wide images using $73\times73$ pixels, with pixel size at 0.2" unchanged. The increased 
image dimension is due to the varying sizes of galaxies in the \descwl catalog. A larger image
is needed to fully contain those galaxies with larger angular sizes. We again find results consistent 
...
All PSFs are Gaussian. 
\jz{(I.e., except for the PSF sizes, all setups are identical to those of case 1 in 
Section~\ref{sec:mc}.)}
...
$\sim200\times$ deeper than the wide-field image. \jz{(I.e., the setups are identical to 
those in case 1 of Section~\ref{sec:mc} except for deep noise level.)} We again measure the
...
both the wide- and deep-field images. \jz{All images are simulated on $73\times73$ pixels 
with size 0.2"} With this setup, we compute the fractional scatter
